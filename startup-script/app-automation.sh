...
sudo aws s3 cp s3://three-tier-web-application-configurations-466261189876/apache-config/000-default.conf /etc/apache2/sites-enabled/
...
sudo aws s3 cp s3://three-tier-web-application-configurations-466261189876/database-config/wp-config.php /var/www/wordpress/
